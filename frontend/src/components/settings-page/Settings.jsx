...
const Settings = () => {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.put(`https://mini-link-management-platform-backend-deploy.vercel.app/api/users/update`, {
        username: name,
        email,
        phone: mobile
      }, {
  ...
  const handleDeleteAccount = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://mini-link-management-platform-backend-deploy.vercel.app/api/users/delete`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
