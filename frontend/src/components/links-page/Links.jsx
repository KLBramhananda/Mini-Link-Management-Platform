...
const Links = forwardRef((props, ref) => {
  ...
  const handleCreateLink = async (linkData) => {
    try {
      ...
      // Use the short URL generated by the backend
      const updatedLink = {
        ...newLink,
  ...
  // Load initial data and set up click count updates
  useEffect(() => {
    ...
    // Function to update click counts without replacing entire links array
    const updateClickCounts = async () => {
      try {
        ...
        setLinks((prevLinks) => {
          ...
          if (JSON.stringify(updatedLinks) !== JSON.stringify(prevLinks)) {
    ...
    // Update click counts immediately and then every 5 seconds
    updateClickCounts();
    const interval = setInterval(updateClickCounts, 3000);
