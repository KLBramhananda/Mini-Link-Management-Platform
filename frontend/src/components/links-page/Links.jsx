...
const Links = forwardRef((props, ref) => {
  ...
  const handleCreateLink = async (linkData) => {
    ...
    try {
      await axios.post(`https://mini-link-management-platform-server.vercel.app/api/links/create`, newLink); // Revert to original URL
  ...
  const handleLinkClick = async (shortLink) => {
    try {
      const response = await axios.get(`https://mini-link-management-platform-server.vercel.app/api/links/click/${shortLink}`); // Revert to original URL
      const updatedLinks = links.map(link =>
